    ...
    <main class="w-4/5 p-6 bg-slate-50 min-h-screen w-full">
        ...
        <table class="min-w-full bg-white rounded-lg">
            ...
            <tbody>
                ...
                <tr>
                    <td class="py-1 px-3 border-b text-sm text-center">{{ $tenant->id }}</td>
                    <td class="py-1 px-3 border-b text-sm text-center">{{ $tenant->name }}</td>
                    <td class="py-1 px-3 border-b text-sm text-center">{{ $tenant->domain }}</td>
                    <td class="py-1 px-3 border-b text-xs text-center">{{ $tenant->created_at->format('M d, Y') }}</td>
                    <td class="py-1 px-3 border-b">
                        <div class="flex justify-end space-x-2">
                            ...
                            <form action="{{ route('tenants.destroy', $tenant->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Are you very sure you want to delete this tenant?')" class="px-2 py-1 text-sm text-red-500 rounded hover:underline">
                                    Delete
                                </button>
                            </form>
                            <x-modal.editTenantModal :tenant="$tenant"></x-modal.editTenantModal>
                        </div>
                    </td>
                </tr>
                @endforeach
